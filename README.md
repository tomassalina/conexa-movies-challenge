...
- [x] **List movies** — `GET /api/v1/movies`
- [x] **Movie details** — `GET /api/v1/movies/:id`, restricted to
      authenticated users
- [x] **Create a movie** — `POST /api/v1/movies`, admin only
- [x] **Update a movie** — `PATCH /api/v1/movies/:id`, admin only
- [x] **Delete a movie** — `DELETE /api/v1/movies/:id`, admin only
- [x] **SWAPI sync endpoint** — `POST /api/v1/movies/sync`, admin only
...
- **Soft delete** — `DELETE /api/v1/movies/:id` sets `deletedAt`/`deletedBy` instead
  of removing the row.
- **Sort-field whitelist** — `GET /api/v1/movies?sortBy=...` is validated against a
...
- **Manual sync endpoint *and* an automatic cron** — the brief asked for an
  endpoint *or* a cron; this ships both (`POST /api/v1/movies/sync` plus a
  daily `@nestjs/schedule` job).
- **API versioning, a SWAPI-style root, and a real health check** — every
  route lives under `/api/v1/...` (`@nestjs/common`'s built-in URI
  versioning), `GET /` and `GET /api` return a SWAPI-style
  (swapi.tech) map of this API's own resources, and `GET /health` pings
  the actual database via `@nestjs/terminus` instead of returning a
  static "ok" (decision #11 in `ARCHITECTURE.md`).
...
- `POST /api/v1/auth/signup` always creates the user with `Role.USER` — any
  client-supplied `role` field is silently discarded (the signup schema
  doesn't declare one).
- `PATCH /api/v1/users/:id/role` (the only way to promote a user to admin)
  itself requires the `users:manage_role` permission, which only an
  existing admin holds.
...
created by an existing admin via `PATCH /api/v1/users/:id/role`.
...
Every route lives under `/api/v1/...`, except for a handful of
version-neutral, unprefixed endpoints: the discovery routes (`/`, `/api`,
`/api/v1`), the health check (`/health`), and Swagger itself
...
Every route except `POST /api/v1/auth/signup` and `POST /api/v1/auth/login`
requires a bearer token. Obtain one by logging in:
...
| --- | --- | --- |
| **Discovery** | `GET /`, `GET /api`, `GET /api/v1` | Public — SWAPI-style map of this API's resources |
| **Health** | `GET /health` | Public — real database ping via `@nestjs/terminus` |
| **Auth** (`/api/v1/auth`) | `POST /signup`, `POST /login` | Public |
| | `GET /me` | Any authenticated user |
| **Users** (`/api/v1/users`) | `PATCH /:id/role` | Admin only (`users:manage_role`) |
| **Movies** (`/api/v1/movies`) | `GET /`, `GET /:id`, `GET /:id/characters`, `GET /:id/planets`, `GET /:id/species`, `GET /:id/starships`, `GET /:id/vehicles` | Any authenticated user |
| | `POST /`, `PATCH /:id`, `DELETE /:id` | Admin only (`movies:write`) |
| | `POST /sync` | Admin only (`movies:sync`) — triggers a full SWAPI catalog sync |
| **Favorites** (`/api/v1/favorites`) | `POST /:movieId`, `DELETE /:movieId`, `GET /` | Any authenticated user, scoped to their own favorites |
...
```
src/
├── main.ts              # Bootstrap: helmet, CORS, global Zod validation pipe, /api prefix + URI versioning, Swagger setup
├── app.module.ts         # Root module; registers the global JWT + permissions guards
├── app.controller.ts     # SWAPI-style resource discovery (GET /, GET /api, GET /api/v1)
├── auth/                 # Signup/login, JWT strategy, @Public/@Permissions/@AnyAuthenticatedUser decorators, role→permission mapping
├── users/                # User entity, role management (PATCH /api/v1/users/:id/role)
...
├── health/               # GET /health — real database check via @nestjs/terminus
